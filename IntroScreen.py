...
class IntroScreen(QWidget):
    ...
    def __init__(self, displayDuration=2000, fadeOutDuration=1000):
        ...
        # Create the StarLogo widget as a child of this intro screen
        self.StarLogo = StarLogoWidget(self)

        # Hardcoded to start fully visible
        self.opacity = 1.0
        ...
        # Setup fade-out timer with original logic
    ...
    def paintEvent(self, widgetEvent):
        ...
        titleOffsetY = int(min(SCREEN_WIDTH, SCREEN_HEIGHT) * 0.05)   # Vertical offset from center for titles

        smallTextOffsetY = int((titleOffsetY + 3 * titleSpacing) * 1.05)  #Spacing below titles
        subtitleOffsetY = smallTextOffsetY + int(SCREEN_HEIGHT * 0.1)   #Y offset of subtitle


        shadowColor = QColor(0, 140, 255, 140)  #Text shadow color

        # Fonts sized relative to widget height for responsiveness
        ...
        subtitleText = "Abstract hexagonal strategy games"
        ...
        offsetListForTitles = [(1,1),(2,2),(3,3),(4,4)]
        offsetListForSubtitles = [(1,1), (2,2), (3,3)]
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() == Qt.Key_Escape:
            #Call Introfinished early   
            print("DEBUG: INTRO FINISHED (skipped)")
            self.IntroFinished.emit()
